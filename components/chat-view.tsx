...
export default function ChatView(): JSX.Element {
	...
	const starterPrompts = [
		"What events are happening this week at Juniata?",
		"Help me map a 15-credit schedule for next term.",
		"Compare CS, data, and bio-related POE options.",
	...
	return (
		<div className="relative w-full min-h-dvh flex flex-col overflow-hidden text-default-foreground bg-background">
			<div
				...
				style={{
					background: "radial-gradient(900px circle at 18% 18%, rgba(59,130,246,0.18), transparent 46%), radial-gradient(700px circle at 82% 10%, rgba(16,185,129,0.12), transparent 42%)",
				}}
			/>
				...
				{!hasMessages ? (
					<section className="absolute inset-0 overflow-hidden px-4 sm:px-8 pt-6 pb-44 sm:pb-32">
						<div className="absolute inset-0 pointer-events-none opacity-70" style={{
							background: "radial-gradient(620px circle at 25% 12%, rgba(59,130,246,0.18), transparent 58%), radial-gradient(620px circle at 78% 16%, rgba(14,165,233,0.14), transparent 58%)",
						}} />

						<div className="relative mx-auto h-full w-full max-w-5xl flex flex-col items-center justify-center text-center">
							<p className="text-xs uppercase tracking-[0.22em] text-sky-400 font-semibold">AlfieAI Chat</p>
							<h2 className="mt-3 text-[clamp(1.5rem,4vw,2.5rem)] sm:whitespace-nowrap font-semibold leading-tight text-foreground px-2">
								What can I help you discover at Juniata today?
							</h2>
							<p className="mt-3 max-w-2xl text-default-500 text-sm sm:text-base px-2">
								Ask anything about academics, campus life, schedules, events, or policies.
							</p>
							...
							<div className="mt-6 w-full max-w-4xl px-2">
								<InputBar
									...
									onSubmit={(value: string, files: File[]) => {
